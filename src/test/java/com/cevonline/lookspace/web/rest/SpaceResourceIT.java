...
public class SpaceResourceIT {
     ...
     *
     * @see com.cevonline.lookspace.repository.search.SpaceSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void searchSpace() throws Exception {
        ...
        // Search the space
        restSpaceMockMvc.perform(get("/api/_search/spaces?query=id:" + space.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(space.getId().intValue())))
            .andExpect(jsonPath("$.[*].rooms").value(hasItem(DEFAULT_ROOMS)))
