...
export class SpaceComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.spaceService
        ...
        .subscribe(
          (res: HttpResponse<ISpace[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInSpaces(): void {
    this.eventSubscriber = this.eventManager.subscribe('spaceListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ISpace[] | null, headers: HttpHeaders, page: number): void {
