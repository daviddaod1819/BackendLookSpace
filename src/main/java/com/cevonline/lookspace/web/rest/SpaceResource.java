...
import com.cevonline.lookspace.service.SpaceService;
...
/**
 * REST controller for managing {@link com.cevonline.lookspace.domain.Space}.
 */
@RestController
@RequestMapping("/api")
public class SpaceResource {
    ...
    public ResponseEntity<Space> updateSpace(@RequestBody Space space) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, space.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<List<Space>> searchSpaces(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Spaces for query {}", query);
        Page<Space> page = spaceService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
        }
}
